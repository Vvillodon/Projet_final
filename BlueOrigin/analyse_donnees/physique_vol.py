...
class PhysiqueVol(PlanVol):
    ...
    def __init__(self, masse_payload : int, data: list):
        ...
        self.plan_vol_final={'Event': ['Ignition', 'Liftoff', 'MECO', 'Apogee', 'Deploy brakes', 'Restart ignition', 'Touchdown'], 'Elapsed Time(s)': [0, 8.000040292739868, 145.00068020820618, 248.00118017196655, 407.0019702911377, 427.00207018852234, 448.002170085907], 'Altitude (m)': [1118.743806391023, 1119.3699683938175, 57774.66587874014, 106740.76328460686, 5968.729973513633, 1907.2613385310397, 1106.0021588746458]}
        self.data = data
    def deltaV_burnout(self):
        t_MECO=self.plan_vol_final.get('Elapsed Time(s)')[2]
        indice_MECO=self.time.index(t_MECO)
        deltaV_ecef_meco=np.sqrt(self.x_cartesian[indice_MECO] ** 2 + self.y_cartesian[indice_MECO] ** 2 + self.z_cartesian[indice_MECO] ** 2)
